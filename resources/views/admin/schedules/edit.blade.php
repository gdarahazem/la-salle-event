            ...
            <form action="{{ route("admin.schedules.update", [$schedule->id]) }}" method="POST"
                ...
                <div class="form-group {{ $errors->has('day_number') ? 'has-error' : '' }}">
                    ...
                    <input type="number" id="day_number" name="day_number" class="form-control"
                           value="{{ old('day_number', isset($schedule) ? $schedule->day_number : '') }}" step="1"
                           required>
                ...
                <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="title" name="title" class="form-control"
                           value="{{ old('title', isset($schedule) ? $schedule->title : '') }}" required>
                    @if($errors->has('title'))
                        <p class="help-block">
                            {{ $errors->first('title') }}
                        </p>
                ...
                <div class="form-group {{ $errors->has('subtitle') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="subtitle" name="subtitle" class="form-control"
                           value="{{ old('subtitle', isset($schedule) ? $schedule->subtitle : '') }}">
                ...
                <div class="form-group {{ $errors->has('speaker_id') ? 'has-error' : '' }}">
                    ...
                    <select name="speaker_id" id="speaker" class="form-control select2">
                        @foreach($speakers as $id => $speaker)
                            <option
                                value="{{ $id }}" {{ (isset($schedule) && $schedule->speaker ? $schedule->speaker->id : old('speaker_id')) == $id ? 'selected' : '' }}>{{ $speaker }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group {{ $errors->has('event_id') ? 'has-error' : '' }}">
                    ...
                    <select name="event_id" id="event_id" class="form-control select2">
                        @foreach($events as $id => $event)
                            <option
                                value="{{ $id }}" {{ (old('event_id', $schedule->event_id) == $id) ? 'selected' : '' }}>{{ $event }}</option>
                        @endforeach
                    </select>
